...
| `draft`        | boolean, 기본 `false`                              | `true`면 비공개                                  |
...
| `legacyUrls`   | string array, 기본 `[]`                            | 빈 값 및 전체 컬렉션 중복 검사                   |
| `sourceStatus` | `verified`, `legacy-placeholder`, `inventory-only` | `verified`만 공개 후보                           |
...
`scripts/verify-content.mjs`와 `pnpm verify:content`를 추가했다. YAML frontmatter를 읽되 production loader 밖의 script로만 동작하며 fixture Markdown을 컬렉션에 만들지 않는다. 검증 스크립트 내부 self-test가 정상 entry, draft, nonverified, root GitHub URL, incomplete/complete study의 공개 여부를 확인한다.
...
4. 각 entry의 canonical slug와 기존 URL 대응표
...
- canonical slug와 legacy URL 충돌 검사가 가능함
