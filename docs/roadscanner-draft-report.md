...
개발 환경에서만 preview가 생성된다. 프로덕션 공개 조건인 `draft: false`와 `sourceStatus: verified`를 충족하지 않으므로 목록, 홈과 상세 route에서 모두 차단된다.
...
메인 페이지에서는 Hero의 `작업 보기`·`소개`와 헤더의 Design·Projects·Study·Posts·About을 각각 `#design`·`#about`과 콘텐츠 섹션 anchor에 연결했다. 일반 모션 환경에서 `scroll-behavior: smooth`와 80px scroll margin이 적용되며, reduced-motion 환경에서는 기존 정책대로 즉시 이동한다.
...
`docs/roadscanner-content-review.md`의 체크리스트에서 제목, 설명, 기간, 상태, 사용자 역할, 기술 스택과 저장소 링크를 먼저 확인한다. 이후 Owner가 공개 가능한 실제 화면 원본을 제공하고 사실관계와 역할 표현을 승인한 별도 단계에서만 `sourceStatus: verified` 전환을 검토한다. 대표·상세 미디어 검수와 최종 공개 승인 전에는 `draft: true`를 유지한다.
