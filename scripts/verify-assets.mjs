...
const v0SourcePrefix = '<design-reference>/public/'
const configuredV0PublicRoot = process.env.V0_REFERENCE_PUBLIC?.trim() || null
const v0PublicRoot = configuredV0PublicRoot
  ? resolve(configuredV0PublicRoot)
  : null
...
async function main() {
  ...
  assert(
    ...
    'Manifest does not cover every legacy img file',
  )

  const v0SourcePaths = new Set()
  const v0Hashes = new Set()
  const v0SourceAvailable =
    v0PublicRoot !== null && (await isDirectory(v0PublicRoot))
  ...
  for (const [index, entry] of v0ReferenceAssets.entries()) {
    ...
    assert(
      entry.sourcePath.startsWith(v0SourcePrefix),
    ...
    if (v0SourceAvailable) {
      const sourceRelative = entry.sourcePath.slice(v0SourcePrefix.length)
      assert(
        sourceRelative.length > 0 &&
          !sourceRelative.startsWith('/') &&
          !sourceRelative.includes('\\') &&
          !sourceRelative.split('/').includes('..'),
        `${label}: invalid source-relative path`,
      )
      const sourceAbsolute = resolve(v0PublicRoot, sourceRelative)
  ...
  if (v0SourceAvailable) {
    const actualV0Files = (await walkFiles(v0PublicRoot))
      .filter((file) => imageExtensions.has(extname(file).toLowerCase()))
      .map((file) =>
        toPosixPath(`${v0SourcePrefix}${relative(v0PublicRoot, file)}`),
      )
